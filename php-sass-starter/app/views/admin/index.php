...
<html lang="de">
  <head>
    <meta charset="UTF-8">
    <title><?php echo htmlspecialchars($data['title'] ?? 'Admin-Panel')?></title>
    ...
    <div class="main-layout">
      <section class="main-content">
        ...
        <?php if (! empty($_SESSION['admin_user_create_error'])): ?>
          <p style="color:#c0392b; font-weight:700;"><?php echo htmlspecialchars($_SESSION['admin_user_create_error'])?></p>
...
-->
        <p>
          <a class="btn-primary" style="text-decoration:none; display:inline-block; margin-right:.5rem;" href="/index.php?action=admin_projects">Projekte (Verwaltung)</a>
          <a class="btn-primary" style="text-decoration:none; display:inline-block;" href="/index.php?action=admin_user_new">+ Benutzer hinzufügen</a>
        </p>
...
<?php $role = $u['role'] ?? 'calisan'; ?>
                <tr>
                  ...
                  <td><?php echo htmlspecialchars(($u['adi'] ?? '') . ' ' . ($u['soyadi'] ?? ''))?></td>
                  ...
                  <td><?php echo htmlspecialchars($u['son_giris_tarihi'] ?? '-')?></td>
                  <td>
                    <form method="post" action="/index.php?action=admin_update_role" style="display:flex; gap:.4rem; align-items:center;">
                      <input type="hidden" name="id" value="<?php echo (int) $u['id']?>" />
                        ...
                        <option value="calisan" <?php echo $role === 'calisan' ? 'selected' : ''?>>Mitarbeiter</option>
                        <option value="musteri" <?php echo $role === 'musteri' ? 'selected' : ''?>>Kunde</option>
                        </select>
